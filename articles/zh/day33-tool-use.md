...
核心区别在于最后一行——**MCP 是第一个"为模型读"而设计的协议**。REST 和 RPC 的消费者是人类程序员：人读文档、写胶水代码、调试接口。MCP 的消费者是 AI 模型：模型通过 `tools/list` 自主发现可用工具，通过 JSON Schema 理解参数结构，通过 description 字段的自然语言理解工具语义，然后直接构造调用。
