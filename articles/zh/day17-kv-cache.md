...
>
> **"per-channel"（逐通道）和 "per-token"（逐 token）是什么意思？**
>
> 想象 K_cache 矩阵，形状是 (n_tokens x d_k)。每行是一个 token，每列是一个维度（"通道"）。
>
> - **Per-channel** = 对每一**列**（维度）分别量化。每个维度有自己的缩放因子。这样可以处理不同维度数值范围差异大的问题（比如 dim_1 因为 RoPE 总是在 2.0 附近，而 dim_0 在 -0.5 到 0.3 之间）。
> - **Per-token** = 对每一**行**（token）分别量化。每个 token 有自己的缩放因子。
>
> **类比：** 你要给 10 个班级的学生成绩归一化。Per-channel = 每个科目（数学、英语、科学）单独归一化。Per-token = 每个学生单独归一化。如果某个科目所有人都考 90+ 而另一个科目在 40-60 之间，按科目归一化保留更多信息。
...
- **光互联**：研究使用基于光的芯片间连接，大幅增加**芯片之间**的带宽（不是芯片内部）。注意：这跟 9.1 节提到的芯片内内存墙是不同的瓶颈——9.1 说的是 GPU 核心与 HBM 之间的带宽（芯片内部），而光互联解决的是多 GPU 或 chiplet 架构中芯片间的通信瓶颈。
