...
在稠密模型里，每个 token 都会经过同一个 FFN，所以这一层里的几乎所有参数，在每一步都会收到梯度信号。

但在 MoE 层里，每个 token 只会激活极少数 expert，比如 128 个 expert 中只选 top-2。这意味着：
- 只有被选中的 expert 会收到这个 token 的梯度；
- 没被选中的 expert，对这个 token 来说几乎收不到任何梯度；
- 而且这些更新是由一个仍在学习中的 router 动态决定的。

所以，稀疏梯度的根源就是 **条件激活（conditional activation）**：对任意一个 token 来说，大多数 expert 参数根本没有出现在它的计算路径上。

这会让优化更困难，因为更新会变得：
- **不均匀**：有些 expert 吃到大量样本，有些却很少；
- **高方差**：同一个 expert 这一步可能收到很多 token，下一步却几乎没有；
- **自增强**：早期更常被选中的 expert 会拿到更多梯度，学得更快，之后又更容易继续被选中。

这也是为什么 MoE 的训练，尤其在早期路由还没稳定下来之前，往往比稠密模型更不稳定。

#### 怎么判断某个 expert 是否欠训练？

这是一个很实际的问题。在大型 MoE 系统里，有些 expert 可能吃到大量 token，训练得越来越成熟；另一些 expert 可能接收到的样本太少，长期处于欠训练状态。

严格来说，业内并没有一个公认的单一“expert 饱和度”指标，但通常会同时监控一组 proxy：

| 信号 | 它反映什么 |
|---|---|
| **token 负载 / 路由频率** | 这个 expert 是否真的看到了足够多的训练样本 |
| **梯度范数 / 更新活跃度** | 它的参数是否持续收到有效学习信号 |
| **按 expert 统计的 loss / perplexity** | 它在自己负责的 token 上表现得怎么样 |
| **路由稳定性** | 它是否逐渐形成了稳定 specialization |
| **ablation 影响** | 关闭这个 expert 后，模型是否出现可测的退化 |

这里有一个很关键的细节：**低使用率不等于低价值。** 有些 expert 可能专门处理稀有但困难、或者少见但高价值的 case。所以，真正有意义的判断通常是把下面几件事放在一起看：
- 曝光量够不够，
- 参数更新够不够，
- specialization 是否稳定，
- 去掉它之后会不会明显变差。

这也是为什么 MoE 的监控不只是“负载是否均衡”，还包括“每个 expert 是否真的学成了一个有用角色”。
