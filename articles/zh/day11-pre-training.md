...
| | **因果 LM (GPT)** | **掩码 LM (BERT)** | **前缀 LM (T5)** |
|---|---|---|---|
| **预测目标** | 下一个 token | 被遮盖的 token `[M]` | 前缀之后的目标 token |
| **可见上下文** | 左侧所有 token | 双向上下文 | 完整前缀 + 已生成的左侧 |
| **方向** | 左→右（自回归） | 双向 | 编码器双向 + 解码器自回归 |
| **示例输入** | The cat sat on **___** | The [M] sat on the [M] | Q: What is X? \| A: ___ |
| **示例输出** | mat | cat, mat | Y |
| **优势** | 擅长生成 | 擅长理解 | 同时擅长理解和生成 |
| **局限** | 看不到未来上下文 | 不适合生成任务 | 架构更复杂 |
...
> **什么是 logits？** logit 向量是模型最后一层输出的原始、未归一化的分数——词表中每个 token 对应一个分数，可以是任意实数（正或负）。通过 **softmax** 函数将其转换为概率分布（所有值在 0 到 1 之间，总和为 1）。在 PyTorch 中，`F.cross_entropy(logits, targets)` 内部会自动处理 softmax + 负对数似然计算，所以你只需要直接传入原始 logits 即可。
...
![四种优化器在相同损失曲面上的对比](../en/images/day11/optimizers-comparison.jpg)
*图：不同优化器如何导航同一个损失曲面——从 SGD 的锯齿形到 Adam 的平滑收敛。*

想象你被蒙上眼睛站在山顶，要找到最低的山谷。你唯一的工具是感受脚下的坡度（梯度）。

#### SGD（无动量）—— 只看脚下走

公式：θ_{t+1} = θ_t - η · ∇L

每一步都严格跟随当前的坡度。问题在于：在狭长的山谷里，两侧的坡度很陡，但沿着山谷方向的坡度很平缓。你会左右撞墙：
...
v_t = β·v_{t-1} + ∇L(θ_t)，然后 θ_{t+1} = θ_t - η·v_t

β ≈ 0.9 意味着新速度 = 90% 旧速度 + 10% 当前梯度。就像一颗保龄球滚下山坡：
- 左右弹跳的力相互抵消（方向交替）
- 沿山谷方向的力不断叠加 → 越滚越快
...
将学习率除以平方梯度的滑动平均值。梯度大的方向 → 步子小。梯度小的方向 → 步子大。

```
陡峭方向：大梯度 → 除以大数 → 小步 → 不再弹跳
平缓方向：小梯度 → 除以小数 → 大步 → 快速前进
```

#### Adam = 动量 + RMSProp —— 穿自适应鞋的保龄球

两者结合：动量决定方向，自适应调整步长。几乎所有 LLM 训练都用 Adam 或 AdamW。

#### 优化器对比表

| 优化器 | 比喻 | 优点 | 缺点 |
|---|---|---|---|
| SGD | 只看脚下走 | 简单，泛化好 | 来回震荡，速度慢 |
| SGD+动量 | 保龄球 | 冲过小障碍 | 固定步长 |
| RMSProp | 自适应鞋 | 步长自适应 | 没有动量 |
| Adam | 穿自适应鞋的保龄球 | 快速且稳定 | 2倍内存开销 |

### 3.5 现代优化器变体

| 优化器 | 核心思想 | 何时使用 |
|---|---|---|
| **AdamW** | 解耦权重衰减 | LLM 训练标准选择 |
| **AdaFactor** | 内存高效的 Adam | 显存紧张时 |
| **Lion** | 基于符号更新 | 新兴替代方案 |
| **Muon** | Newton-Schulz 正交化动量 | 2024年新算法，比 AdamW 数据效率高 2 倍，优化器显存少 33% |
...
![学习率调度：预热 + 余弦退火](../en/images/day11/cosine-decay-lr.png)
*图：典型的余弦退火学习率调度。预热阶段学习率线性上升，之后按余弦曲线平滑下降。*
...
![香农熵可视化](../en/images/day11/shannon-entropy.png)
*图：香农熵衡量分布的不可预测性。偏斜分布熵低（可预测），均匀分布熵最大（最不可预测）。*
...
![交叉熵直觉：猜得差 vs 猜得好](../en/images/day11/cross-entropy-intuition.jpg)
*图：当你的预测 Q 接近真实分布 P 时，交叉熵低；当两者差异大时，交叉熵高——你一直在被「惊吓」。*
