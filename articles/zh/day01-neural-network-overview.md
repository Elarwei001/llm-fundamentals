...
> **这个公式就是 optimizer 在做的事！**
> 
> 代码里 `optimizer.step()` 执行的正是这个：`θ = θ - η · ∇L`。优化器存储参数和它们的梯度，然后应用这个更新规则。
> 
> 不同的优化器用的是这个公式的变体：
> - **SGD**：就是 `θ = θ - η · ∇L`
> - **SGD + Momentum**：加了速度项，像球带着惯性往下滚
> - **Adam**：根据梯度历史自动调整每个参数的 η（所以叫"Adaptive"）
> 
> Adam 流行是因为它自动调整学习率——不用太费心调 η。
...
> 
> 拆解一下：
> - `np.random.randn(fan_in, fan_out)` → 从标准正态分布采样（均值=0，方差=1）
> - `fan_in` → 这一层的输入连接数（比如 MNIST 第一层是 784）
> - `* np.sqrt(2 / fan_in)` → 把随机数缩小
> 
> **为什么要乘 √(2/fan_in)？**
> 
> 信号流过多层时，方差会不断相乘。如果每层输出方差 > 1，就会爆炸；< 1 就会消失。
> 
> 数学上：如果输入方差是 1，权重方差是 2/fan_in，那输出方差 ≈ 1（ReLU 会把一半值变成 0，所以是"2"而不是"1"）。这样信号在各层之间保持稳定。
> 
> "He"初始化里的 He 是**何恺明**——就是发明 ResNet 的那位！
...
> | 阶段 | 目的 | 需要梯度？ |
> |------|------|-----------|
> | **训练** | 更新权重来改进模型 | ✅ 需要（梯度告诉我们怎么更新） |
> | **测试/推理** | 评估性能或做预测 | ❌ 不需要（权重已固定） |
> 
> 训练时，PyTorch 构建"计算图"来跟踪每个操作如何影响输出——这是反向传播计算梯度所必需的。这个图会占用内存。
> 
> 测试时，我们只想要输出结果。`torch.no_grad()` 告诉 PyTorch："不用跟踪操作了，我不会调用 backward()"。这省了内存，所以测试时 `batch_size` 可以更大。
...
设损失函数为交叉熵，定义误差信号并计算梯度：

$$
\begin{aligned}
L &= -\sum_k y_k \log \hat{y}_k \quad &\text{(交叉熵损失)} \\
\delta^{(l)} &= \frac{\partial L}{\partial z^{(l)}} \quad &\text{(误差信号)} \\
\delta^{(L)} &= \hat{y} - y \quad &\text{(输出层)} \\
\delta^{(l)} &= (W^{(l+1)})^T \delta^{(l+1)} \odot \sigma'(z^{(l)}) \quad &\text{(隐藏层)}
\end{aligned}
$$
...
设输入 x 有 n 个元素，每个元素方差为 Var(x)。权重 W 的每个元素**独立同分布（i.i.d.）**，均值为 0，方差为 Var(W)。

> **什么是 i.i.d.？**
> 
> i.i.d. = **i**ndependent and **i**dentically **d**istributed（独立同分布）
> - **独立**：每个权重的值不影响其他权重
> - **同分布**：所有权重都从同一个分布里采样
> 
> 简单说就是："所有权重都是从同一个骰子独立掷出来的"。

$$
\begin{aligned}
z &= \sum_{i=1}^{n} W_i x_i \quad &\text{(线性层输出)} \\
\text{Var}(z) &= n \cdot \text{Var}(W) \cdot \text{Var}(x) \quad &\text{(输出方差)} \\
\text{Var}(W) &= \frac{1}{n} \quad &\text{(保持方差不变)}
\end{aligned}
$$
...
> **什么是"非凸"？**
> 
> **凸函数**像一个碗：只有一个最低点，梯度下降一定能找到它。
> 
> **非凸函数**有很多坑坑洼洼：有很多局部最小值（是低点但不是全局最低），梯度下降可能卡在任何一个里面。
> 
> 神经网络的损失函数是非凸的，因为有非线性激活（ReLU 等）以及层与层之间的复合。几十亿参数的损失"地形"有无数山丘和山谷。但奇怪的是，SGD 通常能找到好的解——我们不完全理解为什么！
- 为什么过参数化反而有助于泛化
- 神经网络内部学到了什么"表示"

> **过参数化与泛化**
> 
> - **过参数化（Overparameterization）**：模型参数数量远远超过训练样本数量。GPT-3 有 1750 亿参数，但训练数据"只有"约 3000 亿 token。
> - **泛化（Generalization）**：模型在**没见过的数据**上表现如何。泛化好 = 训练集和测试集都表现好；泛化差（过拟合）= 训练集很好，测试集很差。
> 
> **谜团**：传统统计学认为参数太多 → 记住训练数据 → 泛化差。但实际上，更大的模型往往泛化更好！这个反直觉的现象叫 **double descent**（双下降），是近年热门研究方向。
...
```
for each epoch:
    gradient = compute_gradient(所有训练样本)  # 用整个数据集
    weights = weights - learning_rate * gradient  # 每个 epoch 更新一次
```
- 用全部数据计算一个梯度
- 梯度很准确，但很慢（每轮完整遍历才更新一次）
- 占内存：需要把所有数据放内存

**随机梯度下降（SGD）**：
```
for each epoch:
    for each mini-batch (比如 64 个样本):
        gradient = compute_gradient(mini_batch)  # 用小批量
        weights = weights - learning_rate * gradient  # 频繁更新
```
- 用小的随机（"stochastic"）样本来估计梯度
- 每步不那么准，但更新次数多得多
- 实际上收敛更快！

### 为什么 SGD 的"噪声"反而有帮助？

每个 mini-batch 给出的梯度估计略有不同。这种噪声：
1. **帮助逃离鞍点**：随机的"踢一脚"可以把你推出平坦区域
2. **帮助逃离差的局部最小值**：可能弹出浅的山谷
3. **起到正则化作用**：噪声防止过拟合训练数据
...
|--------|------|----------|
| SGD | `θ = θ - η·∇L` | 基础更新 |
| Momentum | `v = βv + ∇L; θ = θ - η·v` | 像滚球一样累积速度 |
| RMSprop | 根据梯度历史自适应调整每个参数的 η | 陡的维度不要冲过头 |
| Adam | Momentum + RMSprop 结合 | 两全其美 |
...
这给出一个 U 型曲线：随着模型复杂度增加，测试误差先降后升。

**但深度学习打破了这个规律！**

研究者发现，如果继续增加参数，超过"过拟合"区域后，神奇的事情发生了：测试误差开始**再次下降**！
...
这叫 **double descent（双下降）**：
1. **第一次下降**：经典区域，参数越多拟合越好
2. **峰值**：插值阈值（参数数 ≈ 训练样本数），泛化最差
3. **第二次下降**：过参数化区域，参数越多反而泛化越好！

**为什么会这样？** 我们不完全理解，但理论包括：
- 更大的模型找到"更简单"的解（范数更低）
- 更多参数 = 更平滑的损失曲面 = 更容易优化
- SGD 的隐式正则化

这就是为什么 GPT-3 有 1750 亿参数还能泛化得好——我们已经深入第二次下降区域了！

---

*Day 1 of 60 | LLM Fundamentals*
*字数：约 5500 字 | 阅读时间：约 25 分钟*
