...
> #### 延伸问答：既然这些标量 / softmax / 控制类操作不适合 GPU，为什么不交给 CPU？
>
> 这是一个非常自然的追问，但在 FlashAttention 里通常并不划算。原因不是 CPU 不会算，而是这些操作和 GPU 主计算路径耦合得太紧。
>
> 第一，**数据太远**。如果把 rolling max、rolling sum、局部重缩放这类中间步骤交给 CPU，就需要把中间状态从 GPU 显存搬到 CPU 可访问的内存，再搬回 GPU。很多时候，传输和同步的代价比这些标量计算本身还贵。
>
> 第二，**这些操作不是独立任务，而是 attention kernel 内部的紧耦合步骤**。它们往往刚算完就要立刻被下一步 matmul 或归一化使用。如果拆给 CPU，相当于把一条本来连续的 GPU 流水线切断。
>
> 第三，**并行不等于高效**。CPU 和 GPU 并行只有在两边任务相对独立、数据交换不频繁时才特别值，比如 CPU 做数据预处理、日志、调度，GPU 做训练主算子。但 FlashAttention 里的这些辅助计算属于每个 tile 都要频繁参与的内环操作，不属于这种松耦合分工。
>
> 所以更现实的路线不是“把不擅长 tensor core 的部分交给 CPU”，而是：
>
