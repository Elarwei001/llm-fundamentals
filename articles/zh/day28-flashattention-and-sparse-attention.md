...
### 4.1 FlashAttention-1（Tri Dao 及其合作者，2022）
...
### 4.2 FlashAttention-2（延续 Tri Dao 研究线的后续迭代，2023）
...
### 4.3 FlashAttention-3（同一研究线面向 Hopper 的进一步优化，2024）
...
### 4.4 FlashAttention-4（Zadouri 等人，2026 年 3 月）
...
举个非常小的例子。假设一行 attention score 按两个 tile 看到：
- tile 1: `[2, 1]`
- tile 2: `[5, 4]`

先看 tile 1 时，你只知道当前最大值是 `2`，所以会先按“减去 2”的标准记账：
- `2 -> e^(2-2) = 1`
- `1 -> e^(1-2) = e^(-1)`

这时累计和是 `1 + e^(-1)`。

后来看到 tile 2，才发现全局最大值其实是 `5`。那前面 tile 1 按旧标准记下来的那部分账，就必须先改成“减去 5”的新标准：
- 原来的 `1` 要变成 `e^(2-5) = e^(-3)`
- 原来的 `e^(-1)` 要变成 `e^(1-5) = e^(-4)`

也就是说，前面的累计值要统一乘上一个修正因子 `e^(2-5) = e^(-3)`。这一步把旧账改成新账的过程，就是**重缩放**。等重缩放完成后，前面 tile 的结果才能和后面 tile 的结果正确合并。

**它解决什么问题？**
- 降低 softmax 路径中的额外标量操作
- 减少“不是 matmul、但又必须做”的杂项开销
- 让更多时间留给主算子（这里的“主算子”主要指 attention 里最吃吞吐、最依赖 tensor core 的矩阵乘法路径，比如 `QK^T` 和后面的 `PV`）
...
*图 5：不同的稀疏注意力模式。蓝色单元表示计算的注意力；白色单元被跳过。每种模式捕捉关于哪些 token 需要交互的不同结构假设。局部窗口和跨步 / 膨胀式稀疏模式，在连接拓扑上常常很像卷积或膨胀卷积的感受野；但注意力的具体权重仍然是输入依赖、动态计算出来的，这一点和卷积核的固定共享权重不同。*
...
下面把这些模式按“提出者 / 原理 / 适用场景”拆开讲。

#### 5.1.1 局部（滑动窗口）注意力

**代表提出者 / 代表工作**：这类思想非常早就出现过，可以追溯到早期把 Transformer 用到长序列时的局部 attention 设计；在现代 LLM 语境里，最有代表性的工程化落地是 **Mistral AI 的 Sliding Window Attention（2023）**，以及后续 **Gemma** 等模型的采用。

**工作原理**：每个 token 只看自己附近一个固定窗口内的 token，例如只看前后各 `w` 个位置，而不是对整个长度为 `N` 的序列做全连接注意力。

**它解决什么问题？**
- 把复杂度从全注意力的 `O(N²)` 压到 `O(N × w)`
- 当局部依赖很强时，几乎不损失主要效果
- 非常适合和 KV cache、长上下文推理结合

**适用场景**：
- 语言建模里局部上下文占主导的情况
- 代码建模（因为近邻 token 关系很强）
- 长上下文推理里希望稳定控算力的场景

**局限**：单层内无法直接看到很远位置，只能依赖多层堆叠把信息逐步传播出去。

#### 5.1.2 步幅 / 膨胀稀疏注意力

**代表提出者 / 代表工作**：这种思路和卷积里的膨胀卷积很像，在早期长序列 Transformer 里常作为局部模式的扩展出现；它常见于 **Longformer** 一类变体或其他结构化稀疏设计中，而不是总以一个独立“大模型名字”出现。

**工作原理**：不是看连续的一整段局部窗口，而是按固定间隔取样，例如每隔 `s` 个 token 看一个位置。你可以把它理解成：
- 普通局部窗口 = 连续地看近邻
- 膨胀注意力 = 稀疏地采样更远邻域

**它解决什么问题？**
- 用更少边连接更远距离的信息
- 在不完全退回全注意力的情况下扩大有效感受野
- 对周期性、节奏性或重复结构明显的数据更有吸引力

**适用场景**：
- 音乐、时间序列、带周期结构的数据
- 需要兼顾局部与较远固定跨度关系的任务

**局限**：如果真实依赖不是规则周期性的，固定步幅可能会错过重要连接。

#### 5.1.3 全局 + 局部注意力

**代表提出者 / 代表工作**：
- **Longformer**（Iz Beltagy, Matthew E. Peters, Arman Cohan，Allen Institute for AI，2020）
- **BigBird**（Manzil Zaheer 等人，Google Research，2020）

这类工作都抓住一个关键观察：

> 并不是所有 token 都需要全局可见，但少数关键 token 往往应该拥有更大的感受野。

**工作原理**：
- 大多数普通 token 只做局部 attention
- 少数特殊 token 具有全局连接能力
- 在 BigBird 里，还会进一步混合随机连接

例如：
- `[CLS]`、标题 token、问题 token
- 或者人为选定的 summary / sentinel token

都可以被赋予全局访问权。

**它解决什么问题？**
- 在控制复杂度的同时保留跨段、跨页、跨章节的信息汇聚能力
- 比纯局部窗口更适合文档级理解
- 让“少数全局节点”承担远距离路由作用
...
**局限**：全局 token 怎么选、选多少，往往会影响效果，而且不同任务最优设计不一样。

#### 5.1.4 块稀疏注意力

**代表提出者 / 代表工作**：
- **Block-Sparse Transformer**（Rewon Child 等人，OpenAI，2019 的 Sparse Transformer 路线中就强烈强调结构化 sparsity）
- 后续很多高性能 kernel / accelerator 工作也偏爱 block-sparse 设计

**工作原理**：不是按单个 token 决定稀疏连接，而是把注意力矩阵切成很多块（blocks），再决定：
- 哪些块保留
- 哪些块整块跳过

这是一种非常“工程友好”的稀疏方式，因为硬件和内核实现通常更喜欢规则的大块，而不喜欢特别零碎的随机 sparsity。

**它解决什么问题？**
- 比单点随机稀疏更容易高效实现
- 更容易和 GPU / accelerator 的块状数据搬运模式对齐
- 在保证一定结构灵活性的同时，保留较好的 kernel 效率

**适用场景**：
- 结构化数据
- 图像 patch / 视频 patch
- 希望稀疏模式与硬件实现更好对齐的系统设计

**局限**：结构越规则，通常越容易实现，但表达能力也可能越受限制。

#### 5.1.5 随机稀疏注意力

**代表提出者 / 代表工作**：
- **Sparse Transformer**（Rewon Child, Scott Gray, Alec Radford, Ilya Sutskever，OpenAI，2019）
- **BigBird** 也把随机连接作为整体稀疏图的一部分

**工作原理**：除了局部连接外，再给每个 token 随机分配一些额外连接。这样做的想法是：
- 完全局部会让信息传播太慢
- 随机边可以像“捷径”一样缩短图上的路径长度

从图论直觉看，这有点像 **small-world network（小世界网络）**：
- 大部分边是局部的
- 少量随机边负责远距离跳跃

这里的“小世界网络”指的是一种很经典的图结构：虽然大多数节点只和附近少数节点相连，但只要再加入少量远距离“捷径”，整个网络里任意两点之间的平均路径就会显著缩短。放到注意力里，这个类比想说明的是：你不一定需要完整全连接，才能让远距离信息高效传播；只要保留局部结构，再加入少量随机远距连接，信息就可能更快地跨越长序列传播。
...
**适用场景**：
- 极长序列建模
- 希望在不做完整全注意力的前提下保留远距离捷径的任务

**局限**：随机模式虽然理论上有吸引力，但工程实现和可解释性通常不如更规则的局部 / 块稀疏模式。
...
3. **用稀疏注意力实现长上下文泛化**（Peters 等人，被 ICLR 2026 接收）——使用 α-entmax 产生*天然稀疏*的注意力分布（精确零值，而非近似零值），无需手工设计的稀疏模式即可改善长度泛化能力。([论文](https://arxiv.org/abs/2506.16640))
   ...
   这条工作很值得多看一眼，因为它和前面介绍的 Longformer / BigBird / 滑动窗口不太一样。前面那些方法大多是**先人为规定一个稀疏拓扑**，例如“每个 token 只看附近 4096 个 token”或“再额外给几个全局 token 权限”。而这篇工作的思路更像是：

   > **不要先手工规定谁能看谁，而是让注意力分布自己学出稀疏性。**

   核心工具是 **α-entmax**。你可以把它看成 softmax 的一个稀疏版本：
   - **softmax** 会给几乎所有位置分配一个正权重，只是有些极小；
   - **α-entmax** 则可能把很多位置的权重直接压成 **精确的 0**。

   这点很关键，因为“极小但非零”和“真正为零”在长上下文里差别很大。前者意味着你理论上仍然要为很多位置保留一条计算边；后者则意味着模型真的学会了：

   > **这些位置我现在完全不用看。**

   为什么这可能有助于长上下文泛化？一个直觉是：当序列长度从训练时的 4K 扩展到测试时的 32K、64K、甚至更长时，如果模型仍然习惯“给所有位置一点点注意力”，那么分布会越来越稀、越来越难聚焦；但如果模型学会了在内容层面主动把大部分无关位置压成 0，只保留少数真正重要的连接，它在更长序列上就更有机会保持稳定的注意力模式。

   换句话说，这条路线试图解决的问题不是“怎样设计一个好的人工稀疏图”，而是：

   > **能不能让模型自己学会在更长上下文里把注意力变得又稀疏、又有选择性？**

   这对长上下文特别有吸引力，因为它暗示了一个方向：未来也许不需要完全依赖固定窗口、固定全局 token 或手工规则，而可以让模型根据输入内容，自适应地产生稀疏结构。

   换句话说，这条路线的关键就是：**通过把 softmax 换成稀疏归一化函数，让模型在训练过程中原生学出“哪些 token 的 attention 应该直接为 0”，而不是在训练结束后再做额外剪枝或阈值处理。**

   当然，这条路线也有现实问题：
   - 稀疏分布是否稳定、是否好训练？
   - 学出来的稀疏模式是否真的容易被底层 kernel 高效利用？
   - “内容自适应稀疏”能否在系统层面真正转化为推理加速，而不只是分析上更漂亮？

   所以它现在更像一个**很有潜力的前沿方向**：它不只是让 attention 更便宜，更重要的是，它可能帮助模型在超长上下文下保持更好的聚焦能力和长度泛化能力。
...
*字数：约 4.9 万（含公式、代码与注释） | 阅读时间：约 35-45 分钟*
