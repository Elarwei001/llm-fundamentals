...
> **公式里的 $T$ 是什么?** $T$ 就是 Temperature(温度),也就是你调用 API 时设置的那个参数(比如 `temperature=0.7`)。它在分母上,所以控制的是 token 之间的分数差距被放大还是缩小:
> - $z_i$ = 模型给每个候选 token 的原始分数(logit)
> - 低 $T$(如 0.5)= 分数除以一个小数 = 差距被放大 = 模型强烈偏好高分 token
> - 高 $T$(如 2.0)= 分数除以一个大数 = 差距被缩小 = 弱势 token 也有机会
>
> **例子:** 假设模型给两个 token 打分:"cat"=4, "dog"=2
> - T=0.5:exp(8) : exp(4) = 2980:54 = 几乎肯定选 cat
> - T=2.0:exp(2) : exp(1) = 7.4:2.7 = 大概率选 cat,但 dog 也有真实机会
...
> **固定 k 为什么有问题:**
>
> **确定性上下文** -- Prompt:*"法国的首都是"*
> - "巴黎" (0.95), "里昂" (0.02), "马赛" (0.01)...
> - 只有 1-2 个合理候选。如果 k=10,多出的 8 个就是噪声。
>
> **模糊上下文** -- Prompt:*"她看向了"*
> - "天空" (0.15), "大海" (0.12), "时钟" (0.10), "山" (0.09), "画" (0.08)...
> - 很多候选都合理。k=10 可能还不够产生好的多样性。
>
> **问题所在:** k=10 对两种情况一视同仁,无法适应。这就是为什么下一节会介绍 **top-p(核采样)**--它不看"取几个 token",而是看"取到的 token 概率加起来够不够",自动适应上下文。
...
> **重复惩罚怎么工作：** 模型检查哪些 token 已经在生成的文本中出现过，然后在采样前降低它们的 logit（分数）。这让已经见过的 token 更不容易被再次选中。
>
> **API 里常见的两种变体：**
> - **frequency_penalty** —— token 出现次数越多，惩罚越重。"the" 出现了 5 次比出现了 1 次惩罚更重。
> - **presence_penalty** —— 只要出现过就惩罚，不看出来了几次。任何已见过的 token 都受到固定惩罚。
>
> **没有惩罚的例子：** "The cat sat on the mat. The cat looked around. The cat decided to..."（一直重复 "The cat"）
>
> **有惩罚的例子：** "The cat sat on the mat. It looked around briefly, then decided to..."（避免重复相同模式）
>
...
> **这里的"惊讶度"(surprise)是什么意思？** 这是信息论里的概念，不是日常的"惊讶"。
>
> **Surprise = -log(p)**，即一个 token 的"信息量"：
> - 模型非常确定下一个词是 "the"（p=0.9）-> surprise = -log(0.9) = 0.15 -> 低惊讶（意料之中，没什么信息量）
> - 模型觉得 "banana" 只有 p=0.001 -> surprise = -log(0.001) = 6.9 -> 高惊讶（意料之外，信息量大）
>
> **Typical sampling 的做法：**
> - 算出整个分布的"平均惊讶度"（其实就是熵/entropy）
> - 只保留惊讶度接近这个平均值的 token
> - 排除两类：太确定的（无聊）和太意外的（不靠谱）
>
> **一句话：** Typical sampling 不选"最可能的"，也不选"最刺激的"，而是选"最典型的"——信息量刚好在平均水平的 token。
