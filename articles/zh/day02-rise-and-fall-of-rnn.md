...
> **别混淆 $C_{t-1}$ 和 $\tilde{C}_t$！**
> 
> | 符号 | 含义 | 来源 |
> |------|------|------|
> | $C_{t-1}$ | 旧记忆 | 从上一步传过来的 |
> | $\tilde{C}_t$ | 候选新记忆 | 当前 $x_t$ 和 $h_{t-1}$ 算出来的 |
> | $C_t$ | 更新后的记忆 | 旧记忆和新候选的加权组合 |
> 
> $\tilde{C}_t$ 上面的波浪线 (~) 表示"候选/临时"——还没正式写入记忆。
> 
> 可以这样理解：
> - $C_{t-1}$ = "你已经记住的"
> - $\tilde{C}_t$ = "你刚想到的新东西"
...
> **Q: 候选是只有一个，还是有多个？**
> 
> 只有**一个**候选 $\tilde{C}_t$！但它是一个**向量**（比如 256 维）。Input Gate $i_t$ 决定的是**每个维度保留多少**：
> 
> - $\tilde{C}_t$ = [0.8, -0.3, 0.5, ...]
> - $i_t$ = [0.9, 0.1, 0.7, ...]
> - $i_t \odot \tilde{C}_t$ = [0.72, -0.03, 0.35, ...] ← 逐元素筛选
> 
> 不是"从 5 道菜里选 1 道"，而是"一盘自助餐，每样东西自己决定夹多少"🍽️
> 
> **Q: 门 $i_t$ 是训练出来的，还是手动调的？**
> 
> **训练出来的！** 公式 $i_t = \sigma(W_i \cdot [h_{t-1}, x_t] + b_i)$ 意味着：
> 
> | 部分 | 谁决定 |
> |------|--------|
> | $W_i, b_i$ | 训练时学出来的（训练后固定） |
> | $i_t$ 的值 | 每个时间步根据输入**动态计算** |
> 
> 可以这样理解：
> - $W_i, b_i$ = 你的"判断标准"（训练时学会的）
> - $i_t$ = 你对"这条具体信息要不要记"的**实时判断**
> 
> 网络学会的是"什么情况下该记、什么情况下该忘"的**规则**——不是固定的开关值。
...
> |------|------|------|
> | $o_t$ | 门控："输出多少" | 0~1 |
> | $\tanh(C_t)$ | 归一化：把 cell state 压到合理范围 | -1~1 |
> | $h_t$ | 最终输出 | -1~1 |
> 
> $C_t$ 可能很大（累加了很多步）。如果没有 tanh，它可能是 100、-50 这种值，会让后续计算崩掉。
> 
> 类比：
> - $C_t$ = 你脑子里的原始记忆（可能很极端）
> - $\tanh$ = "冷静一下"，把想法整理成正常表达
...
> **Q: Gates 和 Cell State 的训练方式不同吗？**
> 
> 都是用反向传播，但有个关键区别：
> 
> | 部分 | 有可学习参数吗？ | 怎么更新？ |
> |------|----------------|-----------|
> | **Gates** ($f_t, i_t, o_t$) | ✅ 有 ($W_f, W_i, W_o, b$) | 梯度下降更新 $W, b$ |
> | **Cell State** ($C_t$) | ❌ 没有自己的参数 | 是**计算出来的值**，不是参数 |
> 
> $C_t$ 是根据公式算出来的——它是**数据流**，不是模型参数。不同的输入序列会产生不同的 $C_t$。
> 
> 类比：
> - $W_f, W_i, W_o$ = 你的"判断能力"（学会后固定）
> - $C_t$ = 你读这本书时的"临时记忆"（每本书都不同）
> 
> 我们训练的是 **Gates 的参数**（学会什么时候该记/忘）。Cell State 只是按照学到的规则运行的"记忆容器"。
> 
> **Q: 参数是共享的，那每个位置的门值不都一样吗？**
> 
> 不是！因为 $f_t = \sigma(W_f \cdot [h_{t-1}, x_t] + b_f)$ 取决于 **$h_{t-1}$ 和 $x_t$**，它们每步都不同：
> 
> | 时间步 | $x_t$ (当前输入) | $h_{t-1}$ (上一步隐藏状态) | $f_t$ |
> |--------|-----------------|---------------------------|-------|
> | t=1 | "The" | $h_0$ (初始) | 0.3 |
> | t=2 | "cat" | $h_1$ (包含 "The" 的信息) | 0.9 |
> | t=3 | "which" | $h_2$ (包含 "The cat" 的信息) | 0.95 |
> 
> 同一个 $W_f$，但输入不同 → 输出不同！
> 
> $h_{t-1}$ 里**编码了上下文**：看到 "cat" 后，$h$ 里有"这是个名词/主语"的信息。下一步遇到 "which"，门控就知道"保持住主语信息"。
> 
> **$W_f$ 学的是"怎么根据上下文做判断"，$h$ 提供了上下文！**
...
序列长度: 10
原始 RNN 梯度范数: 4.23e-02    ← 还行，能学
LSTM 梯度范数: 2.87e-01        ← 健康
比率 (LSTM/RNN): 6.8x

序列长度: 50
原始 RNN 梯度范数: 1.56e-08    ← 几乎是 0！(0.0000000156)
LSTM 梯度范数: 1.42e-01        ← 依然健康
比率 (LSTM/RNN): 9102564.1x    ← LSTM 好 900 万倍！

序列长度: 100
原始 RNN 梯度范数: 2.31e-17    ← 完全死了
LSTM 梯度范数: 8.93e-02        ← 学习正常
比率 (LSTM/RNN): 3866233766233766.5x  ← LSTM 好 3800 万亿倍！

序列长度: 200
原始 RNN 梯度范数: 0.00e+00    ← 💀 归零，游戏结束
LSTM 梯度范数: 5.21e-02        ← 依然坚挺！
比率 (LSTM/RNN): inf           ← 没法比了
```

> **怎么看这个结果**：观察原始 RNN 那列——随着序列变长，从 0.04 → 0.00000001 → 0，梯度消失了。而 LSTM 始终保持在 0.05-0.3 之间。这就是为什么 LSTM 能处理长序列！
...
2. **GRU 把遗忘门和输入门合并成一个"更新门"。这种简化相比 LSTM 有什么取舍？**（提示：想想精细控制 vs 计算效率）
