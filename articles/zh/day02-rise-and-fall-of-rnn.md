...
## 附录：BPTT（时间反向传播）是怎么算梯度的？

训练时，梯度是如何通过时间步反向流动的？

### 梯度从右往左流

假设在 t=4 有个 loss L₄，我们要算 ∂L₄/∂W：
...
因为 h₄ 依赖 h₃，h₃ 依赖 h₂...，梯度要**一路乘回去**：

```
∂L₄/∂W = ∂L₄/∂h₄ · ∂h₄/∂W                           ← 直接影响
       + ∂L₄/∂h₄ · ∂h₄/∂h₃ · ∂h₃/∂W                 ← 通过 h₃ 间接影响
       + ∂L₄/∂h₄ · ∂h₄/∂h₃ · ∂h₃/∂h₂ · ∂h₂/∂W       ← 更早的间接影响
       + ...
```

### 关键：∂hₜ/∂hₜ₋₁ 连乘

每一步：hₜ = tanh(W·xₜ + U·**hₜ₋₁**)
...
- 如果 |∂hₜ/∂hₜ₋₁| < 1 → 连乘后趋近 **0**（消失）
- 如果 |∂hₜ/∂hₜ₋₁| > 1 → 连乘后趋近 **∞**（爆炸）

100 步的序列就要乘 99 次：0.9⁹⁹ ≈ 0.00003 😱

这是 vanilla RNN 难以处理长序列的根本数学原因——也是为什么 LSTM 的"门"机制（允许梯度不衰减地流动）如此重要。
