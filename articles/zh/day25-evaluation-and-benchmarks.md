...
如果你真的在部署 LLM 系统，公共 benchmark 只能是起点，不能是终点。真正重要的是：它在**你的用户、你的任务、你的失败场景**里到底好不好用。

#### 直觉：从学校考试，转向“在你自己的路上考驾照”

公共 benchmark 更像标准化考试，它告诉你一个司机大体上懂不懂交通规则。

但如果你经营的是出租车公司，你真正关心的是：
- 他能不能应付新加坡的大雨？
- 能不能进机场 pickup lane 不犯错？
- 面对难缠乘客时会不会慌？
- 会不会因为一个小失误造成高额损失？

产品 eval 也是一样。你不只是想找一个“聪明模型”，而是想找一个**在你真实业务环境里稳定成功的模型**。

#### 一个实用的 eval 框架

一个简单但实用的产品 eval 体系，通常有四层：

| 层级 | 你要构建什么 | 它回答什么问题 |
|---|---|---|
| **离线任务集** | 来自真实使用场景的一组固定样本 | “这个模型至少能不能做我们最核心的任务？” |
| **评分 rubric / judge** | 明确的打分规则，人类或 LLM judge | “什么叫好，什么叫勉强可用，什么叫失败？” |
| **在线实验** | A/B test 或 shadow deployment | “它真的改善真实用户结果了吗？” |
| **持续监控** | dashboard、报警和回归检测 | “它是不是随着时间在变差？” |

下面把这四层说具体一点。

#### 第一步：建立你自己的任务集

第一件事是收集**真实用户 query**，而不是自己想象出来的课本题。

例如：
- 如果你在做客服机器人，就收集真实 support tickets；
- 如果你在做代码助手，就收集团队真实的 coding task；
- 如果你在做法务助手，就收集真实 document review workflow。
...
|---|---|
| 正常路径 | 用户清楚地提出一个标准问题 |
| 边界情况 | 缺少上下文、表达模糊、格式奇怪 |
| 高风险场景 | 法律、医疗、金融、安全相关请求 |
| 失败探针 | jailbreak、误导性上下文、对抗性输入 |

一个好的任务集，不是“随便凑 100 个 prompt”，而是**你产品真实运行环境的地图**。

#### 第二步：定义评分规则（rubric）

很多团队会卡在这里：样本收集了，但没有定义“什么叫成功”。

一个有用的 rubric 通常会包含 3-5 个维度，比如：

| 维度 | 你要问的问题 |
|---|---|
| 正确性 | 回答在事实或逻辑上是否正确？ |
| 帮助性 | 它真的解决了用户的问题吗？ |
| 指令遵循 | 它是不是准确完成了要求，不多不少？ |
| 安全性 | 它是否避免了有害或高风险行为？ |
| 格式合规 | 它是否按要求返回了指定结构？ |
...
如果要求是“JSON 格式必须正确”，那二元打分最好。如果你更关心风格、帮助性、自然度，成对比较通常更合适。

#### 第三步：决定谁来打分

通常有三种 judge：

| Judge 类型 | 最适合什么任务 | 主要弱点 |
|---|---|---|
| **人工 judge** | 高风险、细腻、需要上下文理解的任务 | 贵、慢、扩展性差 |
| **规则检查器** | 格式校验、精确匹配、单元测试 | 对开放任务太僵硬 |
| **LLM-as-judge** | 大规模定性评分 | judge 本身也会带偏见 |

现实里，最好的做法通常是**混合**：
- 用规则检查器做客观校验，
- 用 LLM judge 做大规模质量评估，
- 用人工抽查高风险样本和关键切片。

#### 第四步：做在线实验

离线 eval 回答的是“它能不能做”。在线 eval 回答的是“用户会不会因此受益”。
...
这一步很重要，因为一个模型可能 offline 看起来更强，但 online 反而更差。比如：一个模型生成的回答更长更详细，judge 很喜欢，但真实用户觉得太慢、太啰嗦。

#### 第五步：上线后持续监控 drift

Eval 不是一次性活动。模型会 drift，用户行为会变，prompt template 会更新，上游 API 也会改。

所以上线之后，至少要持续追踪这些东西：

| 要监控什么 | 为什么重要 |
|---|---|
| 不同任务类型的质量分数 | 及时发现某个 workflow 的局部退化 |
| 安全失败率 | 提前抓住低频但严重的问题 |
| 拒答率 | 判断模型是不是变得过于保守 |
| 延迟 / 成本 | “更强”的模型可能贵得不划算 |
| 新出现的用户意图 | 防止 eval 集过时 |

#### 一个小团队也能执行的最小框架

如果一个小团队问：“最低可行的 eval 流程是什么？”

我会建议：

1. 先收集 100-300 条真实 query，
2. 分成正常 / 边界 / 高风险三类，
3. 定义一个 4 维 rubric，
4. 用混合 judge 方案，
5. 离线比较两个候选模型，
6. 在线做一个小规模 A/B test，
7. 每周维护一个回归 dashboard。

这已经比“因为某个模型 MMLU 高 2 分就直接选它”强太多了。

公共 benchmark 告诉你模型在实验室里怎么样。
你自己的 eval 才告诉你它在你业务里到底能不能打。
