...
MMLU（Massive Multitask Language Understanding）之所以经典，是因为它覆盖了 57 个学科，从法律到生物到数学。很多年里，它几乎是大家默认会引用的 benchmark。
...
| MMLU-CF | 去污染版本 | 信号更干净 | 仍继承 MMLU 的根本限制 |

它的分数计算非常简单：

$$
	ext{Accuracy} = rac{	ext{答对题数}}{	ext{总题数}}
$$
...
#### 大白话：不是“你背过没有”，而是“你会不会想”

有些 benchmark 试图测的不是记忆，而是真正的推理能力。

- **GPQA** 测的是研究生级别的科学推理。
- **AIME** 测的是奥数级别的数学多步推理。
- **ARC-AGI** 测的是抽象模式归纳和小样本泛化。
...
**HumanEval** 更像给学生一道小编程题：给你函数签名和 docstring，你把函数补出来。

**SWE-bench** 更像给工程师一个真实 GitHub issue：进入一个又大又乱的代码库，真的把 bug 修掉。
...
Arena 不用固定试题，而是让用户把两个匿名模型的回答放在一起比，选更喜欢的那个，最后聚合成 **Elo rating**。
...
#### 大白话：专门用来阻止排行榜通货膨胀的考试

当老 benchmark 饱和以后，社区就会造一个更难的新 benchmark。

**Humanity's Last Exam (HLE)** 本质上就是这样一个东西：大规模、专家撰写，而且故意设计成让前沿模型也很难拿高分。
...
![Figure 3b: Radar chart comparing two models across seven benchmarks](./images/day25/benchmark-radar-comparison.png)
*图 3b：两个假想模型在 7 个 benchmark 上的能力画像。Model A 在 MMLU 和 HumanEval 上很强，但真正能拉开差距的是 SWE-bench 和 HLE。*
