...
近端策略优化（Proximal Policy Optimization，PPO）是 RL 阶段的标准算法。选择它有几个原因：
...
![图 5：基础模型 vs SFT vs SFT+RLHF](./images/day13/base-vs-sft-vs-rlhf.png)
*图 5：RLHF 在安全性（无害性）和指令遵循方面提供了最大的提升。仅 SFT 是必要的，但对于生产级助手来说还不够。*
...
RLHF 的局限性催生了替代方案：
- **DPO（Direct Preference Optimization，直接偏好优化）**—— 完全跳过奖励模型（Day 15 介绍）
- **RLAIF（RL from AI Feedback，来自 AI 反馈的 RL）**—— 用 AI 代替人类进行偏好标注
- **Constitutional AI**—— 自我批评和修正循环
