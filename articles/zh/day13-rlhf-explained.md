...
> **🔍 参考模型（π_ref）到底是什么？** 参考模型字面上就是 SFT 模型的一个**冻结副本**——它的权重在 RL 训练过程中被锁住，永远不会更新。它完全不参与训练。相反，它充当一个**「锚点」**——如果策略（正在训练的模型）在 RL 优化过程中偏离这个锚点太远，就会通过 KL 散度受到惩罚。这就是目标函数中 $\beta \cdot \text{KL}(\pi_\theta \| \pi^{ref})$ 这一项的来源。当前策略和参考模型之间的散度越大，惩罚就越重。
>
> **简单类比：** 参考模型就像「你的起点」。无论你怎么优化和探索，都不能离起点太远。它是一根安全绳，确保模型始终有根有据。

> **🔗 三个阶段是权重的逐步精炼：** 重要的是要理解，这个流程中的每个阶段都以前一阶段的权重作为起点——没有什么是从头训练的。
>
> - **预训练：** 在海量互联网文本上做 next-token prediction → 模型「什么都懂一点」
> - **SFT：** 在高质量指令-回复对上微调 → 模型「学会遵循指令和格式化回答」（这一步可选但强烈推荐）
> - **RLHF/PPO：** 模型已经拥有可用的权重——PPO 只是轻轻地把它推向「人类偏好的方向」
>
> RLHF 中的变化是故意设计得很小的。KL 惩罚确保这只是一个**微调**，而不是重写。打个比方：就像大学教育——预训练 = 长大和学语言，SFT = 上课学专业，RLHF = 工作中前辈给你的反馈，帮你打磨职业素养。
...
> **⚠️ PPO 损失 vs SFT 损失——一个重要区分：** 在 RLHF 阶段，PPO 的裁剪目标是**唯一**被优化的损失函数，**没有**与 SFT 的交叉熵损失做融合。总奖励 $= r_\phi(x,y) - \beta \cdot \text{KL}(\pi_\theta \| \pi^{ref})$，PPO 直接用它来计算梯度并更新策略的权重。这就是为什么灾难性遗忘是一个真正需要担心的问题：RL 只为奖励做优化，而不是为了「保留已有知识」。KL 惩罚是*间接*保护已有知识的手段——通过限制权重偏离 SFT 检查点的距离，它确保模型不会忘记已经学到的东西。
