...
**BitNet**（[论文](https://arxiv.org/abs/2310.11453)，Microsoft Research 于 2023 年 10 月提出）不是另一种普通量化方法，而是一次**训练范式的转变**。这里最容易混淆，所以要把它和 GGUF、AWQ、GPTQ 明确区分开：

- **普通量化方法**（GGUF、AWQ、GPTQ）通常是先训练一个 FP16 模型，再在训练后把权重压缩到 8-bit、4-bit 或更低。模型原本是在高精度空间里学到知识，量化是事后的压缩。
- **BitNet 是原生 1.58-bit 训练**。它不是把已有的 LLaMA、Qwen 模型“压扁”成 1.58-bit，而是在训练时就约束权重只能取三种值：{-1、0、+1}。三值权重需要的理论表示量是 log₂(3) ≈ 1.58 bit，所以它从一开始就在低比特空间里学习。

这件事为什么重要？如果你把一个 FP16 模型量化到 4-bit，本质上是在丢掉原本存在的信息，只是希望损失足够小。BitNet 则不同：模型从训练第一天起就知道自己只能用 {-1、0、+1} 这些权重表达知识，因此它学到的是另一种更适合低比特计算的表示方式。它不是“压缩后的模型”，而是“天生低比特的模型”。

效率优势也来自这个约束。普通 LLM 推理的大量计算是浮点矩阵乘法，也就是 multiply-accumulate。三值权重把乘法大幅简化：权重是 +1 就加，是 -1 就减，是 0 就跳过。因此推理时可以用大量简单的加减法替代昂贵的浮点乘法。[BitNet.cpp](https://github.com/microsoft/BitNet) 在 2026 年 1 月的优化更新中，就利用这个特性让 100B 参数模型能在单个 CPU 上以接近人类阅读速度（5-7 tokens/sec）运行。

**代价**是生态还不成熟。BitNet 模型必须从头训练，不能直接把现有的 LLaMA 或 Qwen 权重转换过去；可用模型数量、工具链成熟度和生产案例也远少于 GGUF/AWQ/GPTQ。它更像是一个很有潜力的新方向：如果原生低比特训练继续成熟，本地部署的硬件门槛可能会被重新定义。
