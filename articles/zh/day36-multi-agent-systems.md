...
### 类别 1：系统设计问题（FC1: System Design Issues）

占比 **约 44%**——这是最常见也最根本的失败类别。问题不在于 agent 能力不足，而在于系统本身的规格设计有缺陷：任务定义模糊、角色边界不清、终止条件缺失。Agent 从一开始就不清楚自己该做什么、做到什么程度算完成。

| 编号 | 失败模式 | 占比 | 说明 |
|------|---------|------|------|
| FM-1.1 | **违反任务规格** (Disobey task specification) | 11.8% | Agent 未能遵守任务指定的约束或要求。例如被要求生成 Python 3 代码却用了 Python 2 语法，或忽略了输出格式要求。 |
| FM-1.2 | **违反角色规格** (Disobey role specification) | 1.5% | Agent 越权行事，做了其他角色应该做的事。例如测试 agent 开始写代码，或 CEO agent 抢了 CTO 的话。 |
| FM-1.3 | **步骤重复** (Step repetition) | 15.7% | Agent 不必要地重复已经完成的步骤。这是单一最常出现的失败模式——agent "忘了"自己已经做过了。 |
| FM-1.4 | **对话历史丢失** (Loss of conversation history) | 2.8% | 上下文被意外截断，导致 agent 无视最近的交互历史，回退到更早的对话状态。常见于上下文窗口溢出时。 |
| FM-1.5 | **未察觉终止条件** (Unaware of termination conditions) | 12.4% | Agent 不知道任务已经完成，继续无意义地运转。或者反过来，没满足条件就停了。 |

**Why：** 这类失败的根源通常是 MAS 设计者的责任——系统提示写得太模糊、工作流定义不完整、或没有设定清晰的完成/停止信号。论文指出，一个设计良好的 MAS 应该能用最少但精确的用户输入来解释高层目标。

---

### 类别 2：智能体间对齐失败（FC2: Inter-Agent Misalignment）

占比 **约 33%**——即使每个 agent 的角色定义完美，它们之间的沟通和协调仍然可能崩溃。Agent 对彼此意图的误读、信息的遗漏、以及推理与行动的脱节，构成了第二大失败类别。

| 编号 | 失败模式 | 占比 | 说明 |
|------|---------|------|------|
| FM-2.1 | **对话重置** (Conversation reset) | 2.2% | Agent 意外重启对话，丢失之前所有上下文和进展。通常与 FM-1.4（历史丢失）关联出现。 |
| FM-2.2 | **未能请求澄清** (Fail to ask for clarification) | 6.8% | Agent 面对不明确或不完整的信息时，不请求补充说明，而是基于错误假设继续执行。 |
| FM-2.3 | **任务脱轨** (Task derailment) | 7.4% | Agent 偏离了原始目标，开始处理无关内容。例如调研 agent 在搜索时被侧链信息吸引，越走越远。 |
| FM-2.4 | **信息截留** (Information withholding) | 0.85% | Agent 掌握了关键信息但未主动共享给其他 agent，导致其他 agent 在不知情的情况下做出错误决策。 |
| FM-2.5 | **忽视他人输入** (Ignored other agent's input) | 1.9% | Agent 无视或未充分考虑其他 agent 提供的输入或建议。例如审核 agent 的修改意见被写作 agent 直接跳过。 |
| FM-2.6 | **推理-行动脱节** (Reasoning-action mismatch) | 13.2% | Agent 的推理过程（"我应该做 X"）和实际执行的行为（实际做了 Y）不一致。Agent "说一套做一套"。 |

**How：** 论文强调，诊断这类失败比 FC1 更复杂。相似的表现（如信息缺失）可能源于不同的根因——是 FM-2.4（截留信息）、FM-2.5（忽视输入）、还是 FM-1.4（历史丢失）？这就是为什么 MAST 需要细粒度的分类，而非笼统的"沟通失败"标签。

---

### 类别 3：任务验证问题（FC3: Task Verification）

占比 **约 23%**——即使前两个类别的问题都被解决了，如果系统没有有效的验证机制，错误仍然会溜过。这类失败的核心是：**谁来检查结果？怎么检查？**

| 编号 | 失败模式 | 占比 | 说明 |
|------|---------|------|------|
| FM-3.1 | **过早终止** (Premature termination) | 6.2% | 在所有必要信息尚未交换或目标尚未达成之前就结束了任务。例如代码还没写完就返回"任务完成"。 |
| FM-3.2 | **无验证或不完整验证** (No or incomplete verification) | 8.2% | 系统完全跳过了对输出的检查，或只做了表面的验证。例如 ChatDev 生成的象棋程序通过了编译检查，但运行时存在严重的游戏规则 bug。 |
| FM-3.3 | **错误验证** (Incorrect verification) | 9.1% | 验证过程存在缺陷，将错误的结果判定为正确。验证 agent 自己犯了错，给出"看起来没问题"的误判。 |

**Why & How：** 论文的一个重要发现是，设有显式验证 agent 的系统（如 MetaGPT、ChatDev）总失败率确实更低——但验证 agent 并非银弹。现有验证 agent 往往只做表面检查（编译通过？有输出？），尽管被提示要做彻底验证。这意味着验证本身也需要被设计得更严格，而不仅仅是"加一个审核者"。
...
1. **FM-1.3 步骤重复** (15.7%) — 最普遍的失败
2. **FM-1.1 违反任务规格** (11.8%)
3. **FM-1.5 未察觉终止条件** (12.4%)
4. **FM-2.6 推理-行动脱节** (13.2%)
5. **FM-3.3 错误验证** (9.1%)
6. **FM-3.2 无验证或不完整验证** (8.2%)
7. **FM-2.3 任务脱轨** (7.4%)
8. **FM-3.1 过早终止** (6.2%)
9. **FM-2.2 未能请求澄清** (6.8%)
10. **FM-1.4 对话历史丢失** (2.8%)
11. **FM-2.1 对话重置** (2.2%)
12. **FM-2.5 忽视他人输入** (1.9%)
13. **FM-1.2 违反角色规格** (1.5%)
14. **FM-2.4 信息截留** (0.85%) — 最少见但最隐蔽

### 实践启示

- **前 5 种模式占总失败的约 62%**，优先解决这些就能获得最大的可靠性提升
- **FC1（系统设计）+ FC2（对齐失败）合计占 79%**——这说明大多数失败可以通过更好的规格工程和协调协议来预防
- 没有万能方案——不同框架的失败模式分布差异很大（如 OpenManus 倾向于步骤重复，HyperAgent 倾向于推理-行动脱节）
- 修复方式也各不相同：有些需要重新设计工作流，有些需要更严格的提示工程，有些需要引入独立的验证机制
