...
> **"无限迭代"是什么意思？**
>
> DPO 在固定数据集上跑固定轮次，然后停止。有一个天花板——模型只能学到数据集里的东西。
>
> PPO 则不断生成新回复、获取新奖励信号。只要奖励模型还在改进、策略还没收敛，就可以继续训练。理论上没有固定终点。
>
> **为什么这对生产环境很重要：** OpenAI 和 Anthropic 这样的公司可以负担在大规模计算集群上跑 PPO 几周甚至几个月，持续改进模型。成本巨大，但回报持续。DPO 在这个场景下无法竞争——数据集用完就到顶了。
