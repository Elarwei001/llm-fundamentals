...
SimPO 不计算 $\log \frac{\pi(y|x)}{\pi_{ref}(y|x)}$，而是计算：

$$\log \pi(y|x) - \frac{1}{|y|} \sum_{t=1}^{|y|} \log \pi(y_t|x, y_{<t})$$

*公式 5：SimPO 用回复自身的长度归一化对数概率替代参考模型比率。*

> **这意味着什么？** SimPO 不再问"策略相对冻结模型变化了多少"，而是问"这个回复的 token 概率是否高于自身平均水平？" 长度归一化（$\frac{1}{|y|}$）确保长回复不会被不公平地偏袒。
