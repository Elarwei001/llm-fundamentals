...
> **具体例子：GRPO 是怎么一步步工作的**
>
> 假设 prompt 是一道数学题：*"$\sqrt{144} + 3$ 等于多少？"*
>
> **第一步：生成一组回复。** 模型生成 4 个不同的回答：
> - 回复 A："我来算一下。$\sqrt{144} = 12$，所以 $12 + 3 = 15$。" → 答案：15
> - 回复 B："$\sqrt{144} = 14$，所以 $14 + 3 = 17$。" → 答案：17
> - 回复 C："$\sqrt{144} = 12$，然后 $12 + 3 = 15$。答案是 15。" → 答案：15
> - 回复 D："$144 + 3 = 147$。" → 答案：147
>
> **第二步：用规则验证器给每个回复打分。** 正确答案是 15，所以：
> - A：正确 → reward = 1.0
> - B：错误 → reward = 0.0
> - C：正确 → reward = 1.0
> - D：错误 → reward = 0.0
