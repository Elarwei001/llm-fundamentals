...
但这里的代价也更高。客服机器人答错了，最多让用户生气；SRE 机器人误判一次 rollback、扩容或切流量，可能直接扩大事故。

然而，SRE Agent 的终极价值不只是"安全地执行缓解动作"——更关键的是在告警风暴中**抽丝剥茧地找到问题根因**。真实事故发生时，监控系统可能同时触发几十上百条告警：CPU 飙升、延迟增加、错误率上升、依赖服务超时、队列堆积……人类 SRE 需要在海量噪声中快速判断哪些是症状、哪个是根因，这个过程往往最耗时也最依赖经验。一个优秀的 SRE Agent 应该能够自主地跨数据源关联（metrics、logs、traces、deploy history、config changes）、多轮钻取（从告警集群 → 缩小到可疑服务 → 关联近期变更 → 定位到具体 commit）、最终构建出完整的因果链。

而这里有一个关键设计判断：**通过约束 Agent 不能执行写操作，可以大幅降低安全边界，从而反过来解放它在读操作上的自主性。** 如果 Agent 只能读、只能分析、只能建议，那就可以放心让它自主地去跨系统调查、多轮推理、关联历史事件，而不用担心它误操作把事故扩大。约束不是为了限制能力，而是为了在这个安全边界内最大化它的调查自主性。

因此，本文讨论的重点不是"LLM 能不能看日志"，而是如何把 LLM 放进一个受约束、可审计、可回滚、有人类接管机制的生产系统——在这个系统里，Agent 既能自主地完成耗时的根因定位工作，又不会因为误判而扩大事故。
