...
世界模型通过 RSSM 强制状态持久性:$h_t$ 是整个历史 $(h_{0:t-1}, s_{0:t-1}, a_{0:t-1})$ 的确定性函数。这意味着潜在状态(近似地)是历史的充分统计量。LLM 没有显式的状态变量--整个历史必须从上下文窗口中重构。这在上下文窗口内效果惊人,但超出窗口时会灾难性地退化,并且没有在有原则的机制在跨轮次时维护持久的信念状态(除非将 KV 缓存视为替代品)。
