...
- **确定性状态**，通过循环携带长期记忆（类似 GRU 隐状态）。它记住“发生了什么”。
- **随机状态**，捕捉当前情况的不确定性。它表示“根据噪声观测，现在可能是什么样”。

更新规则为：

$$h_t = \text{GRU}(h_{t-1},\, s_{t-1},\, a_{t-1})$$

这个确定性循环把记忆向前传播。然后从当前观测推断随机状态：
...
![DreamerV3 架构](./images/day24/dreamer/architecture-v3.png)
*图：DreamerV3（Hafner et al., 2023）的双面板架构。**左面板 (a)**：世界模型学习——观测被编码为随机潜在状态，通过循环动力学传播，并重建。**右面板 (b)**：在想象中训练 actor-critic——从一个真实编码状态出发，actor 提出动作，世界模型预测未来潜在状态，critic 评估它们。策略通过在想象轨迹上反向传播来训练。*

DreamerV2（2021）引入了通过直通梯度的离散潜在变量，用 32 个类别型变量（每个 32 个类别）替代了连续高斯潜在变量——这让潜在空间更具表达力，在 Atari 游戏上表现显著提升。DreamerV3（2023）证明了一个固定超参数配置就能在 150+ 个不同基准上达到最优——这在之前的基于模型的强化学习中从未见过。

从 Ha & Schmidhuber 到 Dreamer 的架构演进线是：分离的 VAE + RNN → 端到端变分训练 → 确定性+随机性双路径状态 → 离散潜在变量 → 通用超参数。每一步都让学到的模拟器更鲁棒、更通用。
