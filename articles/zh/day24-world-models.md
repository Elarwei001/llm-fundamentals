...
对于 RSSM（Dreamer 的架构），转移因子进一步拆分：先计算确定性循环状态 $\bar{s}_t$，再从以它为条件的先验 $p_\theta(s_t \mid \bar{s}_t)$ 中抽取随机状态。
...
**解决方案：** 与其找*完美*的解释，不如训练一个神经网络（编码器）来产生一个*足够好*的近似。我们称之为**近似后验** $q_\theta(s_t \mid \bar{s}_t, o_t)$ —— 编码器根据观测和确定性上下文对潜在状态的最佳猜测。

然后我们最大化**证据下界（ELBO）** —— 模型解释数据能力的一个保守估计：

$$\mathcal{L} = \underbrace{\mathbb{E}_{q(s_{1:T})} \left[ \sum_{t=1}^{T} \left( \log p_\theta(o_t | s_t) + \log p_\theta(r_t | s_t) \right) \right]}_{\text{"我的总结能解释证据吗？"}} - \underbrace{\sum_{t=1}^{T} \text{KL}\left( q_\theta(s_t | \bar{s}_t, o_t) \, \| \, p_\theta(s_t | \bar{s}_t) \right)}_{\text{"我的总结是否保持合理？"}}$$

拆开来看三个项：

- **重构**（$\log p_\theta(o_t | s_t)$）：“给定我对当前状态的总结，我能重建我实际看到的东西吗？” 总结好的话，重建图应该跟原图匹配。

- **奖励预测**（$\log p_\theta(r_t | s_t)$）：“给定我的总结，我能预测得到了多少奖励吗？” 这迫使总结保留与任务相关的信息——不只是视觉细节，而是*对游戏来说重要的东西*。

- **KL 正则化**（$\text{KL}(q \| p)$）：“看了证据之后的总结，跟*没看*之前的猜测还接近吗？” 这防止作弊——编码器不能直接把原始观测复制到潜在状态里。它必须产生一个与模型学到的动力学一致的总结。
...
RSSM 的修复：在预测手账旁边加上日记。**确定性路径** $\bar{s}_t = f(\bar{s}_{t-1}, s_{t-1}, a_{t-1})$ 用 GRU 维持稳定、流动的记忆。**随机状态** $s_t$ 只需要在那个稳定上下文的基础上捕捉*当前时刻的不确定性*。
...
| $\beta_c$ | 潜在状态必须多好地预测回合是否继续 | — | 智能体无法规划回合结束 |
| $\beta_{\text{kl}}$ | 后验必须多接近先验 | 后验太泛化（欠拟合） | 后验复制观测（过拟合） |

**DreamerV3 相比早期版本的新东西：**

- **继续标志**（$\text{cont}_t$）：一个小预测器，回答“回合还在继续吗？”这很重要，因为智能体需要知道轨迹什么时候结束——在游戏里死掉和活着是完全不同的。
...
为什么要以确定性状态 $\bar{s}_t$ 为条件？因为编码器不应该每帧都从零开始。如果你的确定性记忆说“你在赛车游戏里”，编码器应该用这个上下文来解释模糊的像素——同一块棕色斑点，在赛车游戏里可能是石头，在冒险游戏里可能是树。
...
这些是简单的小型 MLP（2-3 层），坐在潜在状态 $(\bar{s}_t, s_t)$ 之上，回答两个实际问题：
