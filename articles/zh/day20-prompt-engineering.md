...
> **一个具体例子：** 假设任务是把一篇长新闻分类成 *politics*、*business* 或 *technology*。
>
> 一个比较弱的 prompt 是：*"Read this article and classify it."* 这样模型需要自己决定怎么做，可能会把精力浪费在表面细节上。
>
> 一个更强的 prompt 可以写成：
> 1. 先找出这篇文章讲的核心事件。
> 2. 再找出涉及的关键人物或机构。
> 3. 判断它主要属于 politics、business 还是 technology。
> 4. 如果多个标签都像是对的，用一句话解释歧义在哪里。
> 5. 最后输出标签和置信度。
>
> 这里的作用就是：prompt 先给了模型**结构**（一步一步做），同时把更多**计算预算**分配到了最容易混淆的地方，也就是类别边界和歧义判断，而不是让模型一眼扫过去就仓促下结论。
...
| 失败模式 | 它长什么样 | 为什么会失败 |
|---|---|---|
| **目标太模糊** | “帮我分析一下” | 任务规格不完整。分析什么？风险、情绪、时间线、根因，还是给谁看的版本？ |
| **约束互相冲突** | 同时要求“尽量简洁”又“覆盖所有细节” | 模型只能猜哪条要求更重要 |
| **上下文过载** | prompt 很长，但混进了大量无关材料 | 噪声淹没了真正重要的信号 |
| **没说输出格式** | 你想要表格、JSON、排序结果或引用片段，却没有明确写出来 | 模型只能临场猜格式 |
| **没要求 grounded answer** | 任务本来只允许基于给定文档回答，但 prompt 没明确这一点 | 模型可能把外部常识也混进来，生成看似合理但没有依据的内容 |
| **没有验证环节** | 高风险任务只做一次生成，没有校验 | 错误没有被检查；更安全的做法往往是加入二次验证、工具或多阶段流程 |
...
| 步骤 | 要做什么 | 为什么重要 |
|---|---|---|
| **第一步：先定义“什么叫成功”** | 明确最终答案应该长什么样，哪些错误绝对不能接受 | 如果成功标准不清楚，prompt 就很难清楚 |
| **第二步：先锁定输出格式** | 先把 schema、字段和格式写清楚 | 下游程序或团队成员需要稳定、可预测的输出 |
| **第三步：只加入必要上下文** | 只放那些会改变结论、缩小歧义、提升判断质量的内容 | 额外上下文可能变成噪声，而不是帮助 |
| **第四步：加入约束** | 比如引用原文依据、资料不足时明确说 insufficient information、不得使用外部知识 | 约束能减少常见失败模式，让行为更稳定 |
| **第五步：如果还含糊，再上 few-shot** | 当分类边界微妙、抽取格式复杂或风格要求细时，加入样例 | 样例往往比大段抽象说明更能教会模型 |
| **第六步：用困难样本测试** | 测边界条件、噪声输入、缺失字段、冲突证据和反例 | 只测简单样本会掩盖真正的问题 |
| **第七步：判断这到底是不是 prompt 问题** | 反问自己问题是不是其实出在检索、工具、数据质量或模型能力上 | Prompt 很强，但它不是万能补丁 |
...
| 误区 | 实际情况 |
|---|---|
| **存在一条万能 prompt** | 不存在。prompt 高度依赖任务，也依赖模型 |
| **prompt 越长越好** | 不对。有用信息会帮助模型，无关冗长只会拖累效果和延迟 |
| **模型越来越强，prompt engineering 就会消失** | 只对了一半。低级措辞敏感性可能下降，但规格设计永远不会消失 |
| **prompt engineering 只是换换说法** | 不对。真正重要的是任务设计，包括示例、结构、检索、工具、验证和输出契约 |
