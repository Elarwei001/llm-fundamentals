...
当模型回答 “Who wrote *The Hobbit*?” 这类高频且稳定的事实时，通常很可靠。但如果你问它某个冷门 API 上周的更新、某篇罕见论文里某个表格的页码、或者某个小众人物的完整履历，情况就会弱很多。

因为权重里存的是模糊而分布式的统计记忆，不是带 provenance（出处）的可检索表。

### 3.2 知识会过时

预训练模型知道的，是它训练阶段见过的世界，再加上后续微调时额外注入的一部分信息。训练截止之后发生的变化，它天然不知道。如果没有外部工具，模型只能从旧记忆里猜。

### 3.3 长尾事实更脆弱

高频模式在训练里会被反复强化。稀有事实、冷门名字、边角案例获得的支持则弱得多。这也是为什么模型通常很擅长做宏观解释，但对精确引用、冷门实体、快速变化的数据更容易出问题。
...
训练阶段，经常使用 **teacher forcing（教师强制）**。也就是说，预测第 $t$ 个 token 时，模型前面看到的是正确前缀 $x_{<t}$。但真正生成时，它看到的是自己前面已经生成出来的内容。
