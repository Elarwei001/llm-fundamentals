...
| 解释 | 图示 |
|---|---|
| **“decoding effects” 是什么意思？**<br><br>即使模型内部的概率分布本来就已经有点不稳，**最终怎么解码** 仍然会决定哪个 token 真的被输出。这包括 greedy decoding、temperature、top-k、top-p sampling 等策略。<br><br>如果那个分布里混着一些“看起来合理、但质量不高”的 continuation，比较激进的采样方式就可能把它们放大。也就是说，模型本来只是“不太确定”，而 decoding 会决定这种不确定性最终变成谨慎回答，还是变成一个很像真的 hallucination。<br><br>所以 decoding effects 往往不是最初的根因，更像一个**放大器**：上游问题先产生了弱候选，decoding 再决定这些弱候选会不会真的变成输出，并在后续生成里越滚越大。 | ![图 2：多条路径都可能通向同一种幻觉症状](images/day21/hallucination-causes-map.png)<br><br>*图注：最终暴露出来的是“幻觉答案”，但真正的根因往往来自多个上游环节。* |
...
> **一个很好用的直觉是：** 模型会被推向一种“把当前故事讲圆”的模式。一旦错误说法已经进入前缀，next-token 目标奖励的是局部连贯性，于是后续 token 往往更倾向继续支持这个说法，而不是停下来纠正它。模型不是在有意识地撒谎，但它的行为效果会很像“在自圆其说”。
...
| 缓解手段 | 它做了什么 | 为什么有效 |
|---|---|---|
| **更好的 prompt 和任务定义** | 减少歧义，要求模型基于证据回答、区分 facts 和 assumptions、显式输出不确定性 | 常常能减少不必要的脑补，但本身不能根治幻觉 |
| **Retrieval 和工具使用** | 引入搜索、数据库、文档检索、计算器等外部工具 | 让模型接触现实世界，而不是只依赖参数记忆 |
| **验证与交叉检查** | 生成后再做第二轮检查，对照来源、schema 或显式证据核验 | 可以在输出之后拦截无依据结论 |
| **允许 abstention（保留判断 / 拒答）** | 允许系统说“我不知道”，或输出带不确定性标记的部分答案 | 在高风险领域，比强行给出完整答案更安全 |
| **训练阶段的改进** | 用 grounded data 做 fine-tuning、用偏好优化鼓励诚实、加入 factuality-aware 目标 | 在平均水平上会改善，但仍然只能作为系统设计的补充 |
...
| 误解 | 实际情况 |
|---|---|
| **只要出现幻觉，就说明模型坏掉了** | 不完全对。幻觉更像是把概率语言模型拿去做“用户期待它追求真实”的任务时，自然暴露出来的偏差。只要任务和防护设计得当，模型仍然可以非常有用。 |
| **只要 prompt 写得够好，就能彻底消灭幻觉** | 不能。Prompt 可以减少一些由歧义和上下文混乱引起的问题，但它不能凭空给模型实时世界知识，也不能替代真实验证。 |
| **RAG 已经解决了幻觉** | 也没有。RAG 只是给模型额外证据。检索本身会失败，模型也可能误读、忽略、过度泛化这些证据。 |
| **幻觉只在事实问答里重要** | 不是。任何“无依据细节会带来成本”的场景都重要，比如摘要、代码生成、合规报告、临床记录、agent 调用工具后的总结。 |
...
> **用大白话理解：** 最终答案可以看成两种倾向的混合：
> - 一种是靠模型自己旧记忆 $M$ 来答，
> - 另一种是靠检索到的证据 $R$ 来答。
>
> 如果 $\lambda$ 更大，说明系统更依赖 retrieval。只要检索回来的证据是正确、相关的，这通常会更靠谱。但如果 retrieval 本身拿回来的是噪声、过时内容，或者“看起来很像相关其实不对”的材料，那更大的 $\lambda$ 反而可能让模型**更有依据地答错**，因为它现在更相信坏证据了。
>
> 所以这个公式不是说模型内部真的显式算了一个加权平均，而只是一个**示意性的直觉模型**：RAG 有帮助，是因为它让最终答案更多地被好证据推动，而不是只靠旧的参数记忆。
