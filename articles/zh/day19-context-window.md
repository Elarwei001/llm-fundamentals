...
![图 3：为什么位置扩展需要缩放](images/day19/position-extension-intuition.png)
*图注：如果直接把位置编码外推到远超训练范围的位置，位置相位可能会变形。位置插值或缩放，本质上是把更长的范围压回一个更稳定的区域。*

直觉上可以这样理解：如果模型在 0 到 4K 范围里学会了一套平滑的位置规律，那么你突然让它处理 128K，就像把一根橡皮筋一下子拉得太长。**位置插值（Position Interpolation）**就是把新的长范围压缩映射回模型更熟悉的区域。

当然，这也不是没有代价。压缩虽然帮助稳定，但也可能让远距离位置之间变得更难区分。换句话说，很多“上下文扩展”其实是在用一部分精度换更大的范围。
...
代价也很明显：如果关键信息很远，而且恰好不在这些“全局通路”上，那么模型就更难把它找回来。
