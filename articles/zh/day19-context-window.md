...
![图 7：Long context 不代表每个位置都一样容易检索](images/day19/lost-in-the-middle.png)
*图注：这是一个示意图。常见现象是，位于 prompt 开头或结尾的事实更容易被找回，而埋在中间的信息更容易丢失。*

> **“toy retrieval curves” 是什么意思？**
>
> 它**不是**一种算法，也不是某个正式 benchmark 的名字。这里的 **toy** 意思是“教学用的简化示意图”，**retrieval curves** 指的是：把关键信息放在 prompt 不同位置时，模型成功找回这条信息的概率曲线。
>
> 背后的研究思路很简单：把唯一关键证据分别放在开头、中间或结尾，然后问一个必须依赖这条证据才能回答的问题。把“正确率”随证据位置变化画出来，就得到 retrieval curve。
>
> 所以这张图是对一种**常见实验现象的可视化总结**，不是某篇论文原始数据的精确复刻。
>
> 这条研究脉络里最有代表性的论文之一是 **Liu et al., 2024, _Lost in the Middle: How Language Models Use Long Contexts_**。这篇论文系统展示了一个稳定现象：很多模型对开头和结尾的信息检索更可靠，而对中间区域的信息最容易丢失。
...
一个最常见的评测家族叫 **Needle-in-a-Haystack（大海捞针）** 测试。

思路很简单：
- 准备一段非常长、但大部分无关的上下文（haystack），
- 在里面埋入一条短但关键的信息（needle），
- 再问一个只有找到这条信息才能答对的问题。

**例子：** 在一个 100K token 的 prompt 里藏一句 *"The secret code is 47291"*，然后问 *"What is the secret code?"*

研究者通常会系统地改变几个变量：
- **上下文长度**（8K、32K、128K、1M），
- **needle 的位置**（开头、中间、结尾、随机），
- **needle 的复杂度**（简单字符串、数字、多跳线索），
- **干扰强度**（附近有相似假信息、重复提及、上下文噪声）。

> **重要提醒：** 通过 needle 测试，并不一定说明模型真正“理解”了整个长上下文。有时它只说明模型能把一个埋起来的事实找出来并复制出来。

这也是为什么现代长上下文评测通常会拆成几类：
- **retrieval（检索）**，
- **aggregation（聚合）**，
- **multi-hop reasoning（多跳推理）**，
- **global understanding（全局理解）**，
- **instruction following under long context（长上下文下的指令遵循）**。

| 评测主题 | 它在测什么 | 当前研究进展 | 真实应用案例 |
|---|---|---|---|
| **Retrieval（检索）** | 模型能不能从超长上下文里找出一个关键事实？ | 这是目前最成熟、研究最多的一类。needle-in-a-haystack、passkey retrieval、key-value retrieval 已经成了长上下文模型的标准基本功测试。 | 在法律合同里找某一条款、在病历里找某个日期、在大代码库里找函数定义 |
| **Aggregation（聚合）** | 模型能不能把分散在上下文各处的多个相关事实汇总起来？ | 进展参差不齐。很多模型能找回单个事实，但当它们需要同时收集 10 到 20 个小事实并稳定总结时，表现就明显下降。 | 总结超长会议记录、从聊天历史里提取所有 action items、跨多份文档汇总证据 |
| **Multi-hop reasoning（多跳推理）** | 模型能不能把相距很远的多个事实连起来推理？ | 依然很难。现在很多 benchmark 不只测“能不能记住”，而是测“能不能把 A 段的事实和 B 段的事实拼起来得出答案”。 | 在代码仓库里跨多个文件追踪依赖关系、回答需要连接研究报告两个远距离部分的问题 |
| **Global understanding（全局理解）** | 模型能不能理解超长文档的整体结构、主题或论证主线？ | 比大家想象得更难。模型可能记得一些局部片段，但仍然抓不住整体叙事、篇章结构或核心论点。 | 理解一本书的一章、超长战略备忘录、法院判决书的整体逻辑 |
| **Instruction following under long context（长上下文下的指令遵循）** | 在超长 prompt 里，模型还能不能遵守当前用户指令，并忽略干扰文本？ | 这是非常实际的前沿问题。长 prompt 往往混有旧指令、引用文本、工具输出，模型容易跑偏或听错“谁才是当前真正的指令源”。 | 在长 agent 轨迹里遵守当前任务、只依据最新政策回答、忽略历史上下文中的过时指令 |

除了自定义的 needle 测试，学术界常见 benchmark 还包括 **LongBench**、**RULER** 和 **InfiniteBench**，它们会测试更广泛的长上下文能力，比如问答、摘要、检索和推理。

在工业界，团队通常会把两类测试结合起来：
- **合成测试**：needle-in-a-haystack、passkey retrieval 这类可控实验，
- **真实任务测试**：比如法律搜索、代码库导航、超长客服对话等。

所以真正的经验结论是：**“支持 1M context” 只是容量声明。真正重要的是，模型能不能在这 1M 里稳定地找出信息、基于它推理，并正确使用它。**
...
| 概念 | 它是什么 | 最适合做什么 |
|---|---|---|
| **长上下文** | 模型**当前这一轮**能直接看到的内容 | 在有限文档集合里保持连续性和精确 grounding |
| **检索（RAG）** | 模型外部的系统，在更大的语料库中查找相关内容，再把结果塞回 prompt | 当知识库远大于任何合理 prompt 时，按需取回关键信息 |
| **记忆（Memory）** | 跨轮次、跨会话的持久状态，比如用户偏好、历史摘要、向量数据库、任务状态、长期笔记等 | 长期保存跨时间的状态 |
