...
## 9. Case Study：构建团队 Workflow 系统

让我们走一遍一个真实团队如何从零构建这个系统。假设一个 5 人 platform 团队，有 15 个常见任务：部署、incident response、基础设施 provisioning、日志分析、容量报告等。

### Phase 1：盘点和分类（第 1–2 周）

**Step 1**：列出所有常见任务。每个记录：
- 发生频率（每天、每周、ad-hoc）
- 涉及多少步
- 是固定模式还是需要判断

**Step 2**：按类别分组：
- *标准 workflow*（80% 的情况）：deploy、health check、report——固定模式，理解充分
- *调查性 workflow*（15%）：debugging、root cause analysis——有结构但需要适应性
- *新颖问题*（5%）：前所未有的 incident——agent 自主性至关重要

### Phase 2：构建核心 Skill（第 2–4 周）

从最常见的 5 个任务开始。每个按照 Day 40 的原则设计 skill：
...
每个 skill 有显式的 input/output contract（第 4.1 节）。

### Phase 3：定义 Pipeline（第 4–5 周）

为标准 workflow 把 skill 组合成 pipeline：
...
从 Fixed DAG 模式开始。抵挡住把一切做成动态的诱惑——可预测性才是重点。

### Phase 4：CLI 集成（第 5–6 周）
...
# Agent 在 cloud session 里调用 CLI
$ mycli deploy-and-verify --service api --env production --format json
```

### Phase 5：Cloud 集成和迭代（第 6 周起）

把 CLI 部署到 cloud agent 环境。连接 agent provider，让 pipeline 调用自动拉起 ephemeral session。然后迭代：

- **第 6–8 周**：对标准 workflow 跑 fixed pipeline。监控可靠性。给容易失败的步骤加 checkpoint 和 retry。
- **第 8–10 周**：为调查性 workflow 引入 hybrid 模式。让 agent 在结构骨架内选择诊断 skill。
- **第 10 周起**：度量：团队百分之几的任务由 pipeline 处理？Agent 在哪里仍需人类帮助？相应地演进 pipeline 定义。

### 实践经验

构建过类似系统的团队报告了一致的经验教训：
