...
#### Four major historical threads

Here "stages" is not meant as a perfectly linear year-by-year pipeline. It is more accurate to read them as **four important historical threads**. In particular, **FitNets (2014)** predates **Hinton's 2015 distillation paper**, but it represents the maturation of the intermediate-representation line, while Hinton's paper established the most canonical modern framing of **logit-based KD**.

| Thread | Core idea | Representative authors / institutions | Year | Representative paper |
|---|---|---|---|---|
| **Thread 1: Model Compression** | Train a small model to mimic richer supervision generated by a larger one | **Cristian Buciluǎ, Rich Caruana, Alexandru Niculescu-Mizil**, IBM Research / Cornell and collaborators | 2006 | [Model Compression](https://dl.acm.org/doi/10.1145/1150402.1150464) |
| **Thread 2: Modern logit-based KD takes shape** | Transfer softened logits and dark knowledge through temperature scaling | **Geoffrey Hinton, Oriol Vinyals, Jeff Dean**, Google | 2015 | [Distilling the Knowledge in a Neural Network](https://arxiv.org/abs/1503.02531) |
| **Thread 3: Intermediate representation matching** | Learn not only outputs, but also hidden states / internal features | **Adriana Romero et al.**, Université de Montréal / MILA | 2014 | [FitNets: Hints for Thin Deep Nets](https://arxiv.org/abs/1412.6550) |
| **Thread 4: LLM-era response distillation** | When logits and hidden states are inaccessible, supervise the student directly on teacher outputs | OpenAI, Google, Anthropic, Meta, DeepSeek and broader LLM practice | 2022-2025 | Not one single founding paper, but a convergence of instruction distillation, synthetic data generation, and teacher-output supervision |

#### Thread 1: the precursor was model compression
...
#### Thread 2: Hinton made modern distillation legible
...
- **soft targets**: you can think of these as the **soft probability labels produced after applying softmax, often temperature softmax, to the teacher's logits**. So soft targets are not a completely separate object from softmax. The distinction is conceptual: softmax is the operation, while soft targets are the resulting probability distribution that the student learns from instead of a one-hot hard label. For example, the teacher might assign cat 0.62, dog 0.25, fox 0.10, and car 0.03. This lets the student learn similarity structure between classes, not just the final answer.
...
#### Thread 4: in the LLM era, response-based distillation became dominant in practice
