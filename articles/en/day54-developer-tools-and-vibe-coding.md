...
## 8. Code Example: A Practical Agent Evaluation Harness

The example below shows how a team would evaluate coding-agent output in practice. It is not toy string-matching — it writes agent output to real files, runs actual tests, executes lint checks, verifies importability, and reports structured diagnostics you can act on.
...
Running this harness produces output like:
...
Notice the second scenario: a string-matching eval might confuse `_` with `-`, or pass the check because `.replace()` exists. But the real test fails because `slugify("Hello World")` returns `hello_world` instead of `hello-world`. That is why an eval harness must run code, not inspect what the code looks like.

In a real team, the harness extends further: coverage reports, security scanning (`pip-audit`), performance regression tests, dependency-change detection. The principle stays the same: **agent output is a hypothesis; tests are evidence.**
