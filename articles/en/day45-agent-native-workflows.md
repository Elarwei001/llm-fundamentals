...
## 9. Case Study: Building a Team Workflow System

Let's walk through how a real team might build this system from scratch. Imagine a 5-person platform team with 15 recurring tasks: deployments, incident response, infrastructure provisioning, log analysis, capacity reporting, and more.

### Phase 1: Inventory and Classify (Week 1–2)

**Step 1**: List all recurring tasks. For each, note:
- How often it happens (daily, weekly, ad-hoc)
- How many steps it involves
- Whether it follows a fixed pattern or requires judgment

**Step 2**: Group tasks into categories:
- *Standard workflows* (80% of occurrences): deploy, health check, report — fixed patterns, well-understood
- *Investigative workflows* (15%): debugging, root cause analysis — structured but need adaptability
- *Novel problems* (5%): unprecedented incidents — agent autonomy essential

### Phase 2: Build Core Skills (Week 2–4)

Start with the 5 most common tasks. For each, design a skill following the Day 40 principles:
...
Each skill gets explicit input/output contracts (Section 4.1).

### Phase 3: Define Pipelines (Week 4–5)

Combine skills into pipelines for the standard workflows:

```yaml
# pipelines/deploy-and-verify.yaml
name: deploy-and-verify
steps:
  - deploy → health_check → verify → notify
  - on health_check.failure: rollback → notify
```

```yaml
# pipelines/weekly-report.yaml  
name: weekly-report
steps:
  - gather_metrics → analyze_trends → generate_report → notify
```

Start with Fixed DAG patterns. Resist the temptation to make everything dynamic — the predictability is the point.

### Phase 4: CLI Integration (Week 5–6)

Wrap everything in a CLI:

```bash
# List available pipelines
$ mycli pipelines
deploy-and-verify    Deploy a service and verify health
weekly-report        Generate and distribute weekly metrics report  
investigate          Investigate an issue (agent-routed)

# Run a pipeline
$ mycli deploy-and-verify --service api --env staging

# Agent invokes CLI in cloud session
$ mycli deploy-and-verify --service api --env production --format json
```

### Phase 5: Cloud Integration and Iteration (Week 6+)

Deploy the CLI to your cloud agent environment. Connect the agent provider so that pipeline invocations spin up ephemeral sessions automatically. Then iterate:

- **Week 6–8**: Run fixed pipelines for standard workflows. Monitor reliability. Add checkpoints and retries for failure-prone steps.
- **Week 8–10**: Introduce hybrid patterns for investigative workflows. Let the agent choose diagnostic skills within a structured skeleton.
- **Week 10+**: Measure: What percentage of team tasks are handled by pipelines? Where does the agent still need human help? Evolve the pipeline definitions accordingly.

### Lessons from Practice

Teams that have built similar systems report consistent lessons:
