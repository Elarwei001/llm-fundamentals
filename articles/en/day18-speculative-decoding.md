...
Getting a good draft model is crucial for acceptance rate. Different training strategies exist, each with different trade-offs:

**Approach 1: Use an existing smaller model (no training needed)**
- Simply pair a small off-the-shelf model (e.g., LLaMA-68M) with a large model (e.g., LLaMA-70B) from the same model family.
- Pros: zero training cost, easy to deploy.
- Cons: acceptance rate may be low if the small model is not well-aligned with the target on your specific domain.
- Works best when: both models share the same tokenizer and architecture.

**Approach 2: Knowledge distillation from the target model**
- Train the draft model to mimic the target model's output distribution. The target model acts as a "teacher," and the draft model is the "student."
...
**Approach 4: Domain-specific adaptation**
- Start with a general-purpose draft model and fine-tune it on domain-specific data (e.g., biology, code, Chinese text).
- Research shows that synthetic data generated by the target model (via prompting frameworks like Magpie) can achieve 80-93% of the performance of training on real user queries.
- This is practical when you have a specific deployment domain (e.g., a coding assistant, a medical chatbot).

> **Which approach should you choose?**
>
> - **Quick start**: Use an existing small model from the same family (Approach 1).
> - **Best quality**: Knowledge distillation (Approach 2), especially offline distillation.
> - **Lowest overhead**: N-gram or retrieval-based (Approach 3), good for code/structured text.
> - **Domain-specific**: Fine-tune a general draft model on domain data (Approach 4).
